...
<div id="sit_ov_wrap">
		...
		for ($i=1; $i<=10; $i++)
		{
				...
				$thumb_img .= '<li class="swiper-slide">';
		...
		if ($thumb_img)
		{
				...
				echo '<ul class="swiper-wrapper">'.PHP_EOL;
				echo $thumb_img;
				echo '</ul>'.PHP_EOL;
				echo '</div>'.PHP_EOL;
				echo '<div class="btn_wr"><a href="#" class="pager-prev"><i class="fa fa-angle-left"></i></a><div id="slide-counter"><strong class="slide-index current-index"></strong> / <strong class="slide-index total"></strong></div><a href="#" class="pager-next"><i class="fa fa-angle-right"></i></a> </div>'.PHP_EOL;
				echo '</div>';
		}
		?>
...
<div id="btn_option">
		<div class="sl_option">
				...
				<?php } ?>
				<div id="sit_ov_btn">
						<a href="javascript:item_wish(document.fitem, '<?php echo $it['it_id']; ?>');" id="sit_btn_wish"><span class="sound_only">위시리스트</span><i class="ri-heart-add-fill"></i></a>
						<?php if ($is_orderable) { ?>
			...
			<div class="_item">
				<div class="img">
					<a href="#">
						...
						<span class="mark">국산</span>
					</a>
				</div>
				<div class="info">
					<div class="subject"><a href="#">최상급 급냉 숫꽃게5kg 최상급 급냉 숫꽃게5kg</a></div>
					<div class="price">
...
$(function(){
		...
		const currentEl = document.querySelector('#slide-counter .current-index');
		const totalLeng = $('#sit_pvi_slide .swiper-slide').length;

		const swiper = new Swiper('#sit_pvi_slide', {
			loop: true,                 // bxSlider 기본동작에 가깝게 무한 루프
			autoplay: {
				delay: 3000,
				disableOnInteraction: false
			},
			slidesPerView: 1,
			// pager(false), controls(false)에 대응 → pagination/nav 생략
			...
			on: {
				init(sw) {
					// 총 개수: 루프일 땐 duplicate 제외한 실제 개수를 사용
					if (totalEl) totalEl.textContent = totalLeng;
					// 현재 인덱스: realIndex가 실제 인덱스
